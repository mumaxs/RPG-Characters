...
public class Warrior extends Character{
    public Warrior(String name) {
        super(name);
        this.roleClass = RoleClass.WARRIOR;
        this.baseStats.setWarriorStats();
    ...
    public void equipArmor(Armor armor) throws InvalidArmorException {
        ...
        }else if(armor.getArmorType() == ArmorType.PLATE || armor.getArmorType() == ArmorType.MAIL) {
    ...
    /**
     * Calculate the damage per second of the character. Warrior have a increase in damage by 1% for each strength point.
     * If no weapon is equipped, weapon dps are set to 1.
     * Calculated by; weapon dps * (1 + (total strength / 100 ))
     */
    @Override
    public void calculateDps() {
        if(this.equipped.containsKey(Slot.MAINHAND)){
            this.dps = Math.round((((Weapon) equipped.get(Slot.MAINHAND)).getDps() * (1.0 + (this.primaryAttribute.getCharacterTotalStrength(this.baseStats.getStrength()) / 100.0))));
        }else{
            this.dps = Math.round((1.0 + (this.primaryAttribute.getCharacterTotalStrength(this.baseStats.getStrength()) / 100.0)));
        }
    ...
    public void levelUp() {
        this.level += 1;
        this.baseStats.updateDexterity(2);
        this.baseStats.updateIntelligence(1);
        this.baseStats.updateVitality(5);
        this.baseStats.updateStrength(3);
        this.secondaryAttribute.setHealth(this.baseStats.getVitality());
        this.secondaryAttribute.setArmorRating(this.baseStats.getStrength(), this.baseStats.getDexterity());
        this.secondaryAttribute.setElementalResistance(this.baseStats.getIntelligence());
        updatePrimaryAndSecondaryStats();
