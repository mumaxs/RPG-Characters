...
public class Mage extends Character{
    public Mage(String name) {
        super(name);
        this.roleClass = RoleClass.MAGE;
        this.baseStats.setMageStats();
    ...
    public void equipWeapon(Weapon weapon) throws InvalidWeaponException {
        ...
        } else if (weapon.getWeaponType() == WeaponType.STAFF || weapon.getWeaponType() == WeaponType.WAND) {
    ...
    /**
     * Calculate the damage per second of the character. Mage have a increase in damage by 1% for each intelligence point.
     * If no weapon is equipped, weapon dps are set to 1.
     * Calculated by; weapon dps * (1 + (total intelligence / 100 ))
     */
    @Override
    public void calculateDps() {
        if(this.equipped.containsKey(Slot.MAINHAND)){
            this.dps = Math.round((((Weapon) equipped.get(Slot.MAINHAND)).getDps() * (1.0 + (this.primaryAttribute.getCharacterTotalIntelligence(this.baseStats.getIntelligence()) / 100.0))));
        }else{
            this.dps = Math.round( 1 * (1.0 + (this.primaryAttribute.getCharacterTotalIntelligence(this.baseStats.getIntelligence()) / 100.0)));
        }
    }
    ...
    public void levelUp() {
        this.level += 1;
        this.baseStats.updateDexterity(1);
        this.baseStats.updateIntelligence(5);
        this.baseStats.updateVitality(3);
